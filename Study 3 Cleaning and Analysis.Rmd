    ...
    left_join(
      tmp %>% 
        cohens_d(outcome.var~adhd,
           var.equal=T, ci=T) %>% 
        select(.y., effsize,magnitude,conf.low,conf.high),
      by = ".y.") 
  pw.tmp = pw.btwn %>% 
    mutate(.y. = comp.name) %>% 
...
```{r}
# p_load(effects)
# low.adhd <- mean(df.all$asrs_sum) - sd(df.all$asrs_sum)
# hi.adhd <- mean(df.all$asrs_sum) + sd(df.all$asrs_sum)
# em.obj <- effect(c("asrs_sum*FL_35_DO"),
#                mod = mod, 
#                xlevels = list(asrs_sum=c(low.adhd,
#                                          mean(df.all$asrs_sum),
#                                          hi.adhd)))
# plot(em.obj,multiline = T)
```
