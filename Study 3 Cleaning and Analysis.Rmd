...
cat(paste0("There was a significant effect of ADHD self-identification on ADHD symptom severity scores, ",
           tmp.res$result[1], ". "))
tukeyres <- df.all %>% tukey_hsd(asrs_sum~adhd_yn)
df.all["adhd"] <- if_else(df.all$adhd_yn == "No",
                      "Non-ADHD", "ADHD")
cat(paste0("\"No\" respondents ( _M_ = ", msd$m[1], ", _SD_ = ", msd$sd[1],
           "N = ", msd$n[1],") reported fewer symptoms than \"Yes\" respondents ( _M_ = ",
...
```{r}
write.csv(pw_result, paste0(output, "pairwise_comparisons.csv"))
```
...
```{r}
long.df <- df.all %>% 
  select(adhd, FL_35_DO, all_of(analysis.cols), consc.gen) %>% 
  pivot_longer(cols=all_of(c(analysis.cols, "consc.gen")))
```
...
p.adhd <- long.df %>% 
  mutate(adhd = factor(adhd, levels = c("Non-ADHD", "ADHD")),
         name = str_replace(name, "consc.gen", "Conscientiousness"),
         name = str_replace_all(name, "[.]", "-"),
         name = str_to_title(name)) %>% 
  ggplot(aes(x=adhd, y = value, group=adhd))+
  geom_boxplot(aes(colour=adhd))+
  facet_wrap(~name)+
  theme_bw()+
  theme(panel.grid.major=element_blank(),
          panel.grid.minor=element_blank(),
...
```{r}
p.for <- long.df %>% 
  mutate(FL_35_DO = str_replace(FL_35_DO, "work_att", "Work"),
         FL_35_DO = str_replace(FL_35_DO, "gen_attn", "General"),
         name = str_replace(name, "consc.gen", "Conscientiousness"),
         name = str_replace_all(name, "[.]", "-"),
         name = str_to_title(name)) %>% 
  ggplot(aes(x=FL_35_DO, y = value, group=FL_35_DO))+
  geom_boxplot(aes(colour=FL_35_DO), position="dodge2")+
  facet_wrap(~name)+
  theme_bw()+
  theme(panel.grid.major=element_blank(),
          panel.grid.minor=element_blank(),
...
params.part.scalar <- data.frame(parTable(scalar))
params.part.scalar$eq <-str_c(params.part.scalar$lhs, " ", params.part.scalar$op, " ", params.part.scalar$rhs)
params.part.scalar <- params.part.scalar %>% 
  filter(eq %in% naughty.scalar)
params.part.scalar$est.se <- str_c(number(params.part.scalar$est, accuracy = .01),
                                   " (",
                                   number(params.part.scalar$se, accuracy = .01),
                                   ")")
params.part.scalar$group.f <- ifelse(params.part.scalar$group==1, "NonADHD",
                                     "ADHD")
params.part.scalar <- params.part.scalar %>% 
  select(eq, group.f, est.se) %>% 
  pivot_wider(names_from=group.f,values_from=est.se, id_cols=eq)
write.csv(params.part.scalar, paste0(output, "work_item_ests_pscalar.csv"))
#summary(scalar,fit.measures=TRUE) 
anova(scalar, metric)
scalar.metric.comp <- compareFit(metric, scalar)
...
Refit the model with partial scalar invariance
...
params.part.scalar <- params.part.scalar %>% 
  select(eq, group.f, est.se) %>% 
  pivot_wider(names_from=group.f,values_from=est.se, id_cols=eq)
write.csv(params.part.scalar, paste0(output, "general_item_ests_pscalar.csv"))
anova(scalar, metric)
scalar.metric.comp <- compareFit(metric, scalar)
#save the model comparisons locally
write.csv(data.frame(), 
